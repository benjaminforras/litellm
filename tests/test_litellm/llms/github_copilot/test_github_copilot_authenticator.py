...
class TestGitHubCopilotAuthenticator:
    ...
    def test_get_access_token_from_env(self, authenticator, monkeypatch):
        ...
        with patch(
            "builtins.open", side_effect=AssertionError("should not read token file")
        ), patch.object(
    ...
    def test_get_api_key_refresh_uses_env_access_token(
        ...
        with patch("builtins.open", side_effect=_open_side_effect), patch(
            "litellm.llms.github_copilot.authenticator._get_httpx_client",
            return_value=mock_client,
        ), patch.object(mock_response, "json", return_value=mock_api_key_data), patch(
            "json.dump"
        ):
            api_key = authenticator.get_api_key()
    ...
    def test_get_api_base_missing_file_with_env_token_skips_warning(
        self, authenticator, monkeypatch
    ):
        """Env-token auth should not warn when api-key.json is absent."""
        monkeypatch.setenv("COPILOT_GITHUB_TOKEN", "env-access-token")

        with patch("builtins.open", side_effect=IOError("file missing")), patch(
            "litellm.llms.github_copilot.authenticator.verbose_logger.warning"
        ) as mock_warning, patch(
            "litellm.llms.github_copilot.authenticator.verbose_logger.debug"
        ) as mock_debug:
            api_base = authenticator.get_api_base()
    ...
    def test_get_api_base_missing_file_without_env_token_warns(self, authenticator):
        """Without env-token auth, missing api-key.json should still warn."""
        with patch("builtins.open", side_effect=IOError("file missing")), patch(
            "litellm.llms.github_copilot.authenticator.verbose_logger.warning"
        ) as mock_warning:
            api_base = authenticator.get_api_base()
